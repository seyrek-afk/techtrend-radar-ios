...
struct TrendsView: View {
    ...
    private var listHeader: some View {
        HStack {
            ...
            Spacer()
            if vm.isStale { KeywordBadge(text: "Önbellekten", color: .slateMuted) }
        }
    ...
    private var listContent: some View {
        VStack(alignment: .leading, spacing: 8) {
            if !sectors.isEmpty {
                SectorFilter(sectors: sectors, selected: $selectedSector)
                    .padding(.horizontal, 16)
            }
            VStack(spacing: 8) {
...
struct SectorFilter: View {
    ...
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                FilterChip(label: "Tümü", isSelected: selected == nil) { selected = nil }
            ...
            Text(label)
                ...
                .background(
                    Capsule()
                        ...
                        .overlay(Capsule().strokeBorder(isSelected ? Color.accent.opacity(0.4) : Color.white.opacity(0.06), lineWidth: 1))
                )
        }
    }
}
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                titleView
                metaRow
                if !item.keywords.isEmpty { keywordsRow }
            }
        }
        .padding(.horizontal, 12)
        .padding(.vertical, 12)
    ...
    private var titleView: some View {
        if let urlStr = item.url, let url = URL(string: urlStr) {
            Link(destination: url) {
                Text(item.title)
                    .font(TTFont.body).fontWeight(.medium)
    ...
    private var metaRow: some View {
        HStack(spacing: 6) {
            HStack(spacing: 4) {
                ...
                Text("\(item.score)").font(TTFont.caption).foregroundStyle(Color.slateMuted)
            }
            if let sector = item.sector { KeywordBadge(text: sector, color: .accent) }
        }
    }
...
struct LoadingTrends: View {
    var body: some View {
        ScrollView {
            VStack(spacing: 8) {
                ForEach(0..<10, id: \.self) { _ in SkeletonView().frame(height: 80) }
            }
            .padding(16)
        }
    }
