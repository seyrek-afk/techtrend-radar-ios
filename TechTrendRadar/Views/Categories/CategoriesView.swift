                    ...
                    ScrollView {
                        LazyVStack(spacing: 0, pinnedViews: .sectionHeaders) {
                            ...
                            Section {
                                ...
                                SectionHeader(title: "Tech Kategorileri")
                                    .padding(.horizontal, 16)
                                    .padding(.vertical, 10)
                            ...
                            if !vm.emerging.isEmpty {
                                Section {
                                    VStack(spacing: 10) {
                                        ForEach(vm.emerging) { opp in
                                            EmergingRow(opportunity: opp)
                                        }
                                    }
                                    .padding(.horizontal, 16)
                                    .padding(.top, 8)
                                    ...
                                    SectionHeader(title: "Gelişen Fırsatlar")
                                        .padding(.horizontal, 16)
                                        .padding(.vertical, 10)
                                        .background(Color.bgBase)
                                }
...
struct EmergingRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !opportunity.signalTickers.isEmpty {
                HStack(spacing: 6) {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14)
                .strokeBorder(Color.warning.opacity(0.18), lineWidth: 1)
        )
    }
}
...
struct ConnectorStatusBadge: View {
    ...
    var body: some View {
        ...
        .task {
            health = try? await APIService.shared.health()
        }
    }
}
